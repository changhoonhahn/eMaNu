...
def read_block(filename, block, parttype=-1, physical_velocities=True, no_masses=False, 
        verbose=False, nall=[0,0,0,0,0,0]):
    ''' read data block 
...
def read_gadget_header(filename):
    ...
    head = snapshot_header(curfilename)
    head_dict = {} 
    head_dict['npart']  = head.npart
    head_dict['nall']   = head.nall
    head_dict['a']      = head.time
    head_dict['z']      = head.redshift
    head_dict['masses'] = head.massarr*1e10
    head_dict['boxsize'] = head.boxsize/1e3 # Mpc/h
    head_dict['filenum'] = head.filenum
    head_dict['cooling'] = head.cooling
    head_dict['Omega_m'] = head.omega_m
    head_dict['Omega_l'] = head.omega_l
    head_dict['h']      = head.hubble
    
    rhocrit=2.77536627e11 #h**2 M_sun/Mpc**3
    head_dict['rhocrit'] = rhocrit
    rhocrit /= 1e9 #h**2M_sun/kpc**3
