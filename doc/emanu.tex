...
\begin{abstract}
    ...
    statistics --- \emph{i.e.} the powerspectrum. In this work, we investigate whether we can break 
    these parameter degerancies and tightly constrain $\smnu$ with the next higher-order correlation 
    function, the bispectrum. We first examine the degeneracy between $\smnu$ and $\sig$ using $800$ 
    $N$-body simulations from the HADES suite and demonstrate that the redshift-space halo bispectrum 
    helps break this degeneracy. Next, we quantify the information content of the redshift-space halo 
    bispectrum in a Fisher matrix forecast that includes 
...
$< 10$ and $5\%$, respectively. Hence we conclude that the derivatives have 
...
same.
\ch{talk about how derivative at $\smnu = 0.0$~eV is larger?} 


\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{figs/quijote_Fisher_convergence_dmnu_fin_kmax05.pdf}
