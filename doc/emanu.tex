...
We demonstrated in the previous section, using the HADES simulation, that 
...
bispectrum for an analysis that include cosmological parameters besides 
$\sig$ --- \emph{i.e.} $\Omega_m$, $\Omega_b$, $h$, and $n_s$. Fortunately, 
the Quijote suite of simulations is \emph{specifically} designed to answer 
this question. \ch{Brief restatement of quijote simulation design for 
Fisher forecasting.}

\ch{a sentence or two on fisher forecasting. cite Jungman 1996a,b, Tegmark et al. 1997, dodelson 2003, heavens 2009}. 
Fisher information matrix
\beq 
F_{i,j} = \bigg \langle \frac{\partial^2 \mathcal{L}}{\partial \theta_i \partial \theta_j} \bigg \rangle 
\eeq
\beq 
F_{i,j} = \frac{1}{2}[C^{-1}C_{,i}C^{-1}C_{,j} + C^{-1} M_{i,j}]
\eeq
where
\beq
M_{i,j} = m_{,i} m_{,j}^T + m_{,i}^T m_{,j}
\eeq
We ignore the first term because of \cite{carron2013} so we're left with 
\beq \label{eq:fisher}
F_{i,j} = \frac{1}{2}\bfi{C}^{-1} \left( \frac{\partial B_0}{\partial \theta_i} \frac{\partial B_0}{\partial \theta_j}^T + \frac{\partial B_0}{\partial \theta_i}^T \frac{\partial B_0}{\partial \theta_j} \right).
\eeq
In Figure~\ref{fig:bk_cov}, we present the covariance matrix $\bfi{C}$, which we 
estimate using the $15,000$ Quijote simulations at the fiducial cosmological 
parameters (Table~\ref{tab:sim}). 
