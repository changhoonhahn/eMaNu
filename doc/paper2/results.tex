...
Furthermore, as we note in Section~\ref{sec:hod},
our fiducial HOD paramter values are chosen based on the \cite{zheng2007} fits
to the SDSS $M_r < -21.5$  and $-22$ samples. However, due to the halo mass
limit of {\sc Quijote}, we use a tighter $\sigma_{\log M}$ scatter. This
means that our HOD galaxy catalogs have a different selection function than 
samples based on commonly used $M_r$ or $M_*$ cuts (\eg SDSS or BOSS). To test
the impact of this difference, \ch{in Appendix~\ref{sec:sig}, we compare 
${\partial \Pgl}/{\partial \sigma_{\log M}}$ and 
${\partial \Bg}/{\partial \sigma_{\log M}}$ at $\sigma_{\log M} = 0.2$ (our
fiducial value) to the derivates evaluated at $\sigma_{\log M} = $, estimated 
using higher resolution \quij simulations. Something more after we do the
comparison}.
\ch{do we find a lot of degeneracies that are broken with sigmalogM?}
...
observations~\citep{pujol2014, hearin2016, pujol2017, zentner2019, vakili2019}
and hydrodynamic simulations~\citep{chaves-montero2016, beltz-mohrmann2020}. 
\ch{include \cite{obuljen2019, obuljen2020}}.
...
This velocity bias for centrals reduces the Kaiser effect; while for
satellites, it reduces the FoG effect. For the high luminosity ($M_r < -21$
and $<-21.5$) SDSS samples, \cite{guo2015} find little satellite velocity bias.
...
precision measurements of observations they must be included. Therefore, when
we analyze observations with a simulation-based approach later in the series,
...
beyond the monopole \cite{yankelevich2019} 50\% more information beyond monopole 

we take broad triangle bins, \cite{yankelevich2019} finds higher
...
no geometry obviously. repeat of how realistic geometry lowers the
        signal to noise. 
%Our forecasts are derived using the power spectrum and bispectrum in periodic boxes. We do not consider a realistic geometry or radial selection function of actual observations from galaxy surveys. A realistic selection function will smooth out the triangle configuration dependence and consequently degrade the constraining power of the bispectrum. In Sefusatti & Scoccimarro (2005), for instance, they find that the signal-to-noise of the bispectrum is significantly reduced once survey geometry is included in their forecast. We also do not account for super-sample covariance (e.g. Hamilton et al. 2006; Sefusatti et al. 2006; Takada & Hu 2013; Li et al. 2018), which may also degrade constraints. Survey geometry and super-sample covariance, however, also degrades the signal-to-noise of their power spectrum forecasts. Hence, with the substantial improvement in the Mν constraints of the bispectrum, even with survey geometry we expect the bispectrum will significantly improve Mν constraints over the power spectrum.
