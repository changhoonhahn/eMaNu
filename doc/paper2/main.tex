...
\begin{abstract}
    ...
    paper, we present the constraining power of the redshift-space {\em galaxy} 
    bispectrum, $\Bg$. We construct $195,000$ mock galaxy catalogs from the \quij 
    $N$-body simulation suite using the halo occupation distribution (HOD) model,
    ...
    on $\Om$, $\Ob$, $h$, $n_s$, $\sig$ by \ch{3.3, 3.6, 4.5, 4.9, and $4.7{\times}$} 
    over power spectrum, even after marginalizing over HOD parameters. For $\smnu$, 
    we derive $5.6\times$ tighter constraints with $\Bg$. Even with priors from
    \planck, $\Bg$ improves cosmological constraints by \ch{$\gtrsim 2\times$}. 
