...
\begin{abstract}
    ...
    free-streaming scale and leave an imprint on large-scale structure that can
    be measured to constrain their total mass, $\smnu$. With the standard
    analysis of two-point clustering stastics, $\smnu$ constraints are severely limited by parameter
    degeneracies. \cite{hahn2020} demonstrated that the halo bispectrum, the
    ...
    paper, we present the advantages of analyzing the redshift-space {\em galaxy} bispectrum. 
    We construct $195,000$ mock galaxy catalogs from the \quij suite of
    $N$-body simulations with the halo occupation distribution (HOD) model,
    which provides an effective galaxy bias framework well-suited for simulation-based
    approaches. Using these mocks, we present the Fisher matrix forecasts of $\{\Om$, $\Ob$, $h$, $n_s$, $\sig$, $\smnu\}$ and
    quantify for the first time, the total information content of the
    redshift-space galaxy bispectrum down to nonlinear scales. For
    $\kmax=0.5\hmpc$, with the galaxy bispectrum, constraints on $\Om$, $\Ob$, $h$, $n_s$, $\sig$ improve by 
    \ch{3.3, 3.6, 4.5, 4.9, and 4.7} over power spectrum constraints, even
    after marginalizing over HOD parameters. For
    $\smnu$, we derive $5.6\times$ tighter constraints with the bispectrum.
    Including priors from \planck, the bispectrum improves cosmological constraints 
    by \ch{$\gtrsim 2\times$}. While effects such as survey geometry and
    assembly bias will impact the constraining power for galaxy surveys, these
    ...
    especially $\smnu$, for upcoming galaxy surveys.
\end{abstract}
