...
We measure the bispectrum and parameter forecasts using the same methods as
...
($\Pgl$; left) and bispectrum ($\Bg$; right) of the galaxy catalog at the fiducial
cosmology, constructed using the fiducial HOD parameters (blue). The $\Pgl$ and 
$\Bg$ are averaged over 15,000 $N$-body realizations. In the left panel, we
...
most loop satisfying $k_1 \le k_2 \le k3$. For comparison, we include the
...
\begin{center}
    ...
    and bispectrum monopole ($\Bg$; right) of the galaxy catalog at the
    fidicual cosmology, constructed using the fiducial HOD parameters (blue).
    The $\Pgl$ and $\Bg$ are averaged over 15,000 $N$-body realizations and
    measured using the same FFT-based estimator as \cite{hahn2020}. In the 
    left panel, we plot both the $\Pgl$ monopole ($\ell = 0$; solid) and quadrupole 
    ($\ell = 2$; dashed). In the right panel, we plot $\Bg$ for all 1898 triangle
    ...
    fiducial cosmology (black). We note that the fiducial galaxy catalog has 
...
To estimate the constraining power of $\Bg$, we use Fisher information
...
$7,500$ realizations at $\theta_i^{+}$ and $\theta_i^{-}$, the HOD +
...
We emphasize that by using galaxy catalogs constructed from $N$-body simulations,
we are able to accurately extend our analysis beyond the limitations of 
analytic methods (\eg perturbation theory) down to the nonlinear regime.
