...
non-overlapping redshift bins over $0.5 < z < 2.1$. They use a perturbation
theory model for the redshift-space $\ell = 0, 2, 4$ power spectrum 
multipoles and the bispectrum monopole with a bias model and a Markov-Chain
Monte-Carlo (MCMC) approach to derive their parameter constraints. Rather 
than imposing a $k_{\rm max}$ to limit their forecast to scales where 
their perturbation theory model can be trusted, they use a theoretical 
error covariance model~\citep{baldauf2016} and extend down to $k = 0.5~\mpc$
}

%with a galaxy bias model that has 4 bias 
%and 4 counterterm normalization parameters at each redshift bin. Rather
%than Fisher, they use an MCMC approach to derive the parameter constraints
%
%Such forecasts make 
...
%non-elliptical or asymmetric~\cite{wolz2012}
