...
of the bispectrum, $\bfi{C}$ (Figure~\ref{fig:bk_cov}). Second, the Quijote 
...
content of the redshift-space bispectrum in the non-linear regime. We discuss subtleties 
of the bispectrum derivative and tests of convegence and stability in Appendix~\ref{sec:numerical}. 
...
volume, V, $\sigma_\theta$ scales as $1/\sqrt{V}$. We list the precise marginalized 
Fisher parameter constraints of both cosmological and nuisance parameters for 
$P_0$ and $B_0$ in Table~\ref{tab:forecast}. 
...
SNR measurements cannot be easily compared to Fisher analysis~\citep{}, % https://arxiv.org/abs/1506.00083, https://arxiv.org/abs/1512.05383
...
correlations are no longe rsimilar, which contribute to the constraining
power of $B_0$~(Figure~\ref{fig:bk_fish_05}).

Various difference between our forecast and previous work prevent more 
thorough comparisons. Two crucial aspects, however, distinguish our
...
bispectrum out to nonlinear regimes. Second, we present the first bispectrum 
...
$F_{i,j}$, for $N_{\rm cov} > 5000$ and $< 1\%$ variation in $\sigma_\theta$
for $N_{\rm cov} > 12000$.  For $N_{\rm deriv}$, we find $< 5\%$ variation 
in the $F_{i,j}$ elements and $< 5\%$ variation in $\sigma_\theta$ for 
...
the Fisher matrix. Since Planck constrain $\{\Om$, $\Ob$, $h$, $n_s$, $\sig\}$ 
...
and $B_0$ Fisher matrices. Including Planck priors (\emph{i.e.} adding the 
...
substantial improvements in parameter constraints with the Planck prior, 
the improvement from $B_0$ are numerically robust. Furthermore, $\sigma_\theta$ 
as a function of $k_{\rm max}$ with Planck priors reveal that while the 
...
(Figure~\ref{fig:fish_kmax}), 

Our forecasts are derived using the power spectrum and bispectrum in a 
\emph{periodic box}. We do not consider a realistic geometry or radial selection 
