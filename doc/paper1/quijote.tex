...
\ch{for a realistically forecast of Euclid; however,}
...
and $\smnu$ of a Euclid-like survey~\citep{laureijs2011} in 8 
non-overlapping redshift bins over $0.5 < z < 2.1$. They use
...
and extend down to $k = 0.5~\mpc$. They find $\sim1.4$, 1.5, 1.2, 1.5, and 
...
in the constraints from including $B_0$ than the improvements we find in o
ur $k_{\rm max} = 0.5~\mpc$ $B_0$ constraints over $P_\ell$. 
...
normalization parameters at each redshift bin). Furthermore, the Alcock-Paczynski 
(AP) effect, which they include for $P_\ell$, significantly improves the 
$P_\ell$ parameter constraints (\emph{e.g.} tightens $\smnu$ constraints by $\sim30\%$). 
They however, do not include AP effects in $B_0$. Also, while \cite{chudaykin2019} 
use theoretical error covariance models to extend down to $k_{\rm max} = 0.5~\mpc$, 
well beyond the accuracy of the tree-level $B_0$ model, at $k \gtrsim 0.2~\mpc$ these 
theoretical errors for $B_0$ quickly dominate~\citep[see Figure 1 in][]{chudaykin2019}. 
This suggests that their forecasts do not include the significant constraining 
power we find in $B_0$ on nonlinear scales. Furthermore, the theoretical error 
model they use is derived from the 
tree-level $B_0$ and is only validated against the one and two-loop dark matter 
perturbation theory bispectra, which are only marginally more accurate than the 
tree-level~\citep{lazanu2018}, instead of against $N$-body simulations. Also 
different from our analysis, \cite{chudaykin2019} use an Markov-Chain Monte-Carlo 
(MCMC) approach, which derives more accurate parameter constraints for non-elliptical 
or asymmetric posteriors than our Fisher approach~\citep{wolz2012}. They, however, 
neglect non-Gaussian contributions to both the $P_\ell$ and $B_0$ covariance 
matrices and they also do not include the covariance between $P_\ell$ and 
$B_0$ in their joint constraints. Nonetheless, \cite{chudaykin2019} and our 
...
\ch{We note, however, that the posteriors from~\cite{chudaykin2019}, which are
derived using MCMC, are generally elliptical and symmetric}. 
...
and improve constraints on cosmological parameters~\citep{sefusatti2006, yankelevich2019, chudyakin2019}. 
