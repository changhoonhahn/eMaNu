...
and $\smnu^{+++}$ cosmologies (Eq.~\ref{eq:dbkdmnu}), (2) fiducial, $\smnu^{+}$, 
and $\smnu^{++}$ cosmologies, and (3) only the fiducial and $\smnu^{+}$ 
cosmologies~(Appendix~\ref{sec:numerical}; Figure~\ref{fig:dPBdmnu}). 
...
both $P_0$ and $B_0$. The differences in the derivatives do not impact the 
$\Om$, $\Ob$, $h$, $n_s$, and $\sig$ constraints. While the differences in 
the derivatives impact the $\smnu$ constraints, because the $P_0$ and $B_0$ 
derivatives are impacted by the same factor, the improvement of the $B_0$ 
constraint over the $P_0$ constraints is not impacted. In addition to the 
stability of the derivatives, because we use $N$-body simulations, we also 
examine the convergence of our forecasts in Appendix~\ref{sec:numerical}. 


We refer to Apprendix~\ref{sec:numerical} for a more detailed investigation 
on 
