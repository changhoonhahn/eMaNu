...
\begin{center}
    ...
    \ch{
        ...
        approximations than analytic methods (perturbation theory). \emph{Furthermore, with 
        their accuracy on small scales, these simulation based derivatives enable us to quantify, 
        for the first time, the information content of the bispectrum in the nonlinear regime.} 
    }
    }
\label{fig:bk_deriv}
\end{center}
...
\begin{center}
    ...
    \caption{Fisher matrix constraints for $\smnu$ and other cosmological parameters 
        ...
        factors of \ch{1.9, 2.7, 3.1, 3.6, and 2.3}, respectively. {\em For $\smnu$, the
        bispectrum improves $\sigma_{\smnu}$ from \ch{$0.1653$ to $0.0342$ eV} --- a factor 
...
\frac{\partial B_0}{\partial \smnu} \approx \frac{-21 B_0(\smnu^{\rm fid}) + 
...
a one-loop perturbation theory model for the redshift-space $\ell = 0, 2, 4$ 
power spectrum multipoles and a tree-level model for bispectrum monopole. 
...
and extend down to $k_{\rm max} = 0.5~\mpc$. They find $\sim1.4$, 1.5, 1.2, 1.5, and 
...
in the constraints from including $B_0$ than the improvements we find in 
our $k_{\rm max} = 0.5~\mpc$ $B_0$ constraints over the $P_\ell$ constraints. 
...
the $P_\ell$ and $B_0$ covariance matrices and they also do not include the 
covariance between $P_\ell$ and $B_0$ for their joint constraints. Nonetheless, \cite{chudaykin2019} and our 
...
\ch{
    ...
    accuratey estimate the full high-dimensional covariance matrix of 
    the bispectrum. With these advantages, we present the first forecast
    of the bispectrum down to nonlinear scales of cosmological parameters 
