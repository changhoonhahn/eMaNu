...
\edt{
    ...
    the triangles in the darker shaded region. $\partial \log B_0/\partial \smnu$
    ...
    derivatives with respect to $\Om$, $\Ob$, $h$, and $n_s$, which have significant 
    scale dependence at $k < 0.2~\mpc$. For a given $k_1$ and $k_2$, $\partial \log B_0/\partial \smnu$
    ...
    $\partial \log B_0/\partial \sig$, is scale independent. Meanwhile, 
    $\partial \log B_0/\partial \smnu$ has little $\theta_{12}$ dependence 
    one large scales (low $k_1, k_2, k_3$) and stronger $\theta_{12}$ dependence 
    on small scales (high $k_1, k_2, k_3$). There are also further detailed 
    differences in the shape dependence of $\partial \log B_0/\partial \smnu$ and 
    $\partial \log B_0/\partial \sig$, as well as the other derivatives, highlighted 
    in the shaded regions. The various differences in how $B_0$ responds to the 
    cosmological parameters, and in particular $\smnu$, ultimately allow $B_0$ 
...
\edt{
    ...
    and $\bar{n} \sim 1.56 \times 10^{-4}~h^3{\rm Mpc}^{-3}$, a substantially smaller 
    volume and lower number density than upcoming surveys. 
}
