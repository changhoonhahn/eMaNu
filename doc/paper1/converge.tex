...
\ch{
    We also note that the Quijote simulations with $\smnu > 0$ eV are generated 
    from Zel'dovich approximated initial conditions, while the $\smnu = 0$ eV 
    simulations are generated from 2LPT initial conditions. The difference in 
    initial conditions has a $< 5\%$ impact on the redshift-space $B_0$ and would
    have an even smaller impact on the derivatives.  
}
