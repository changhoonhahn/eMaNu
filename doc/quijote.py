...
def quijote_dPdthetas(dmnu='fin', ratio=False):
    ...
    for tt, lbl in zip(_thetas, _theta_lbls): 
        ...
        if dpdt[klim].min() < 0: 
            sub.plot(quij['k'][klim], np.abs(dpdt[klim]), ls='--', label='-'+lbl) 
        else: 
            sub.plot(quij['k'][klim], dpdt[klim], label=lbl) 
...
def quijote_Fisher_freeMmin(obs, kmax=0.5, rsd=True, dmnu='fin'): 
    ...
    if obs == 'pk': 
        ...
        pks = quij['p0k1'][:,klim] + 1e9/quij['Nhalos'][:,None] # uncorrect shot noise 
    ...
    for par in _thetas: # calculate the derivative of Bk along all the thetas 
        if obs == 'pk': 
            dobs_dti = quijote_dPk(par, rsd=rsd, dmnu=dmnu)[klim] 
...
if __name__=="__main__": 
    ...
    for kmax in [0.5]: 
        ...
        quijote_Forecast_freeMmin('bk_nosqu', kmax=kmax, rsd=True, dmnu='fin')
    ...
    #quijote_Forecast_Nmock('bk', kmax=0.5, rsd=True, dmnu='fin')
