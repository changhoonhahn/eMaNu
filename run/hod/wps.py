...
def write_wp(tt, res='HR', rsd=True, seed=None): 
    ...
    _rbin = 0.5 * (rbins[1:] + rbins[:-1])

    _fwp = fwp(tt, res=res, rsd=rsd, seed=seed) 
    np.savetxt(_fwp, np.array([_rbin, wp]).T) 
    return None 
...
for i in range(10, 20): 
    write_wp(hod_fid, res='LR', rsd=True, seed=i) 
    write_wp(hod_fid, res='HR', rsd=True, seed=i) 
...
for i in range(hod_fid): 
    if i == 0: # Mmin 
        dtt = [-0.15, -0.1, -0.05, 0., 0.05, 0.1, 0.15]
    elif i == 1: # sigma_logM
        dtt = [-0.15, -0.1, -0.05, 0., 0.05, 0.1, 0.15]
    elif i == 2: 
        dtt = [-0.3, -0.2, -0.1, 0., 0.1, 0.2, 0.3]
    elif i == 3: 
        dtt = [-0.3, -0.2, -0.1, 0., 0.1, 0.2, 0.3]
    elif i == 4:
        dtt = [-0.15, -0.1, -0.075, -0.05, 0., 0.05, 0.075, 0.1, 0.15]
    for _dtt in dtt: 
        for ii in range(10): 
            _hod = hod_fid.copy() 
            _hod[i] += _dtt
            _fwp = fwp(_hod, res='LR', rsd=True, seed=i)
            if not os.path.isfile(_fwp): 
                print('writing ... %s' % os.path.basename(_fwp))
                write_wp(_hod, res='LR', rsd=True, seed=ii) 
            _fwp = fwp(_hod, res='HR', rsd=True, seed=i)
            if not os.path.isfile(_fwp): 
                print('writing ... %s' % os.path.basename(_fwp))
