...
################################## INPUT #############################################
thetas = ['logMmin_m', 'logMmin_p', 'sigma_logM_m', 'sigma_logM_p', 'logM0_m', 'logM0_p', 'alpha_m', 'alpha_p', 'logM1_m', 'logM1_p']
#thetas = ['testing']
######################################################################################
step = 48  
time = 24
...
for theta in thetas: 
    ...
    for i in range(int(nodes)): # loop over the different realizations
        a = '\n'.join(["#!/bin/bash", 
            '#SBATCH -p regular',
            '#SBATCH -n 1',
	    '#SBATCH -t %i:00:00' % time,
            '#SBATCH -C haswell',
            '#SBATCH -J B_%s%i' % (theta, i),
            '#SBATCH -o _B_%s%i.o' % (theta, i),
            '#SBATCH -L SCRATCH,project',
            "#SBATCH --mail-type=all",
            "#SBATCH --mail-user=changhoonhahn@lbl.gov",
            "", 
            "module load python", 
            "source activate hod_test", 
            "",
