...
def hod_dBdtheta(theta, z=0, rsd='all', flag=None, silent=True): 
    ...
    fderiv = os.path.join(UT.doc_dir(), 'dat', 
            'hod_dBdtheta.%s%s%s.dat' % (theta, _rsd_str(rsd), _flag_str(flag))) 
    ...
    if theta == 'Mnu': 
        for i_dmnu, dmnu in enumerate(['fin', 'fin0', 'p', 'pp', 'ppp', 'fin_2lpt']): 
            ...
            k1, k2, k3, dbk = Forecast.quijhod_dBkdtheta(theta, 
                    log=False, z=z, dmnu=dmnu, flag=flag, rsd=rsd, Nderiv=None, silent=silent)
            _, _, _, dlogbk = Forecast.quijhod_dBkdtheta(theta, 
                    log=True, z=z, dmnu=dmnu, flag=flag, rsd=rsd, Nderiv=None, silent=silent)
            if i_dmnu == 0: 
                datastack = [k1, k2, k3]
                hdr = 'k1, k2, k3'
                fmt = '%i %i %i'
        ...
        # calculate dB/dtheta and dlogB/dtheta 
        k1, k2, k3, dbk = Forecast.quijhod_dBkdtheta(theta, 
                log=False, z=z, flag=flag, rsd=rsd, Nderiv=None, silent=silent)
        _, _, _, dlogbk = Forecast.quijhod_dBkdtheta(theta, 
                log=True, z=z, flag=flag, rsd=rsd, Nderiv=None, silent=silent)
...
def hod_dP02dtheta(theta, z=0, rsd='all', flag=None, silent=True): 
    ...
    '''
    fderiv = os.path.join(UT.doc_dir(), 'dat', 
            'hod_dP02dtheta.%s%s%s.dat' % (theta, _rsd_str(rsd), _flag_str(flag))) 
